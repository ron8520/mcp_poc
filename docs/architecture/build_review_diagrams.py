...
def architecture_page() -> Page:
    p = Page(
        ...
        width=1000,
        height=1320,
        title="Enterprise MCP Platform - Direct Cedar Target",
        subtitle="One Gateway with separate SharePoint delegated and application permission lanes",
    ...
    p.boxes.extend(
        [
            ...
            Box("employee", 70, 150, 250, 80, "Employee / Claude Code", "Delegated Enterprise MCP token", COLORS["white"], COLORS["purple"]),
            Box("automation", 375, 150, 250, 80, "Application identity caller", "App-only Enterprise MCP token", COLORS["white"], COLORS["purple"]),
            ...
            Box("obo_interceptor", 70, 650, 230, 55, "OBO assertion interceptor", "Delegated lane only", COLORS["white"], COLORS["purple"]),
            Box("user_target", 75, 725, 330, 75, "sharepoint-delegated target", "Graph delegated permission", COLORS["white"], COLORS["green"]),
            Box("auto_target", 595, 725, 330, 75, "sharepoint-application target", "Graph application permission", COLORS["white"], COLORS["green"]),
            Box("user_runtime", 75, 840, 330, 95, "SharePoint delegated Runtime", "Shared image; Graph OBO", COLORS["white"], COLORS["green"]),
            Box("auto_runtime", 595, 840, 330, 95, "SharePoint application Runtime", "Shared image; Graph app token", COLORS["white"], COLORS["green"]),
    ...
    p.edges.extend(
        [
            ...
            Edge("gateway", "obo_interceptor", "validated token", color=COLORS["purple"], source_anchor=(0, 0.75), target_anchor=(1, 0.5), points=((317, 606.25), (317, 677.5))),
            Edge("gateway", "user_target", "SigV4 delegated action", color=COLORS["blue"], source_anchor=(0.25, 1), target_anchor=(1, 0.5), points=((417.5, 675), (430, 675), (430, 762.5))),
            Edge("gateway", "auto_target", "SigV4 application action", color=COLORS["blue"], source_anchor=(0.75, 1), target_anchor=(0.5, 0), points=((582.5, 685), (760, 685))),
            Edge("obo_interceptor", "user_target", "assertion header", color=COLORS["purple"], source_anchor=(0.5, 1), target_anchor=(0.5, 0), points=((185, 715), (240, 715))),
            ...
            Edge("user_runtime", "sharepoint", "Graph OBO", color=COLORS["orange"], source_anchor=(0.5, 1), target_anchor=(0.25, 0), points=((240, 990), (415, 990))),
            Edge("auto_runtime", "sharepoint", "Graph app token", color=COLORS["orange"], source_anchor=(0.5, 1), target_anchor=(0.75, 0), points=((760, 990), (585, 990))),
...
def identity_page() -> Page:
    p = Page(
        ...
        subtitle="Cedar grants tool capability; SharePoint remains authoritative for site and item access",
        output="enterprise-mcp-platform-identity-flow.svg",
    )
    p.boxes.extend(
        [
            Box("flow_caller", 320, 100, 360, 75, "1. Approved caller", "Delegated user or application identity", COLORS["white"], COLORS["purple"]),
            ...
            Box("obo_interceptor", 70, 575, 360, 50, "5A. OBO assertion interceptor", "Validated token; delegated lane only", COLORS["purple_fill"], COLORS["purple"]),
            Box("user_runtime", 70, 660, 360, 85, "6A. SharePoint delegated Runtime", "Reject invalid upload arguments", COLORS["green_fill"], COLORS["green"]),
            Box("auto_runtime", 570, 660, 360, 85, "5B. SharePoint application Runtime", "Reject invalid upload arguments", COLORS["green_fill"], COLORS["green"]),
            Box("obo", 70, 800, 360, 85, "7A. Microsoft Graph OBO", "Delegated Graph token for the employee", COLORS["purple_fill"], COLORS["purple"]),
            Box("app_token", 570, 800, 360, 85, "6B. Graph client credentials", "Dedicated Graph application token", COLORS["purple_fill"], COLORS["purple"]),
    ...
    p.edges.extend(
        [
            ...
            Edge("obo_interceptor", "user_runtime", "assertion + SigV4", color=COLORS["green"], source_anchor=(0.5, 1), target_anchor=(0.5, 0)),
            Edge("flow_cedar", "auto_runtime", "application allow", color=COLORS["green"], source_anchor=(0.75, 1), target_anchor=(0.5, 0), points=((590, 620), (750, 620))),
